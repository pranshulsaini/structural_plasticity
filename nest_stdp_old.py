...
class stdp_class:
    def __init__(self):
        ...
        self.mu_input = 0.5
        self.bg_rate = 2000000 * gaussian(self.xdata, self.mu_input, self.sigma_input)
    ...
    def create_nodes(self):
        self.nodes_inp = nest.Create(self.neuron_model, self.number_input_neurons, params = self.model_params_ex)
    ...
    def connections(self):
        ...
        nest.Connect(poisson, self.nodes_inp, "one_to_one") # default syapses is static_synapse, where weight = 1
        ...
        wt_inp_e =  np.random.rand(self.number_excitatory_neurons, self.number_input_neurons)
        nest.Connect(self.nodes_inp, self.nodes_e, conn_spec = {'rule': 'all_to_all'}, syn_spec = {'model': 'stdp_synapse', 'weight': wt_inp_e , 'delay': 1.0, 'tau_plus': 15})


        #input population to inhibitory neurons.
        wt_inp_i =  (1./5) * np.random.rand(self.number_inhibitory_neurons, self.number_input_neurons)
        nest.Connect(self.nodes_inp, self.nodes_i, conn_spec = {'rule': 'all_to_all'}, syn_spec = {'model': 'stdp_synapse', 'weight': wt_inp_i , 'delay': 1.0, 'tau_plus': 15})


        #recurrent connections from excitatory neurons on itself. Structural
        wt_e_e =    (1. / 5) * np.random.rand(self.number_excitatory_neurons, self.number_excitatory_neurons)
        for j in range(0, 1600):
            wt_e_e[j][j] = 0.0   #prevent from connecting a neuron to itself in case it has randomly been assigned
        nest.Connect(self.nodes_e, self.nodes_e, conn_spec = {'rule': 'all_to_all'}, syn_spec = {'model': 'stdp_synapse', 'weight': wt_e_e , 'delay': 1.0, 'tau_plus': 15})


        # connections from excitatory population to inhibitory population. Simple stdp
        wt_e_i = (1. / 5) * np.random.rand(self.number_inhibitory_neurons, self.number_excitatory_neurons)
        nest.Connect(self.nodes_e,  self.nodes_i, conn_spec = {'rule': 'all_to_all'}, syn_spec = {'model': 'stdp_synapse', 'weight': wt_e_i , 'delay': 1.0, 'tau_plus': 15})


        # connections from inhibitory population to excitatory population. Simple stdp
        wt_i_e =  np.random.rand(self.number_excitatory_neurons, self.number_inhibitory_neurons)
        nest.Connect(self.nodes_i, self.nodes_e, conn_spec = {'rule': 'all_to_all'}, syn_spec = {'model': 'stdp_synapse', 'weight': wt_i_e , 'delay': 1.0, 'tau_plus': 15})


        # connections from inhibitory population to inhibitory population. Simple stdp
        wt_i_i = (1 / 2.5) * np.random.rand(self.number_inhibitory_neurons, self.number_inhibitory_neurons)
        for j in range(0, 400):
            wt_i_i[j][j] = 0.0  # prevent from connecting a neuron to itself in case it has randomly been assigned
        nest.Connect(self.nodes_i, self.nodes_i, conn_spec = {'rule': 'all_to_all'}, syn_spec = {'model': 'stdp_synapse', 'weight': wt_i_i , 'delay': 1.0, 'tau_plus': 15, 'Wmax':0.4})
    ...
    def record_spikes(self):
        ...
        print ('The total number of spikes of the population are', np.sum(self.spikes))
    ...
    def plot_data1(self):
        ...
        pl.savefig('stdpNoise3.eps', format='eps')
    ...
    def plot_data2(self):
        ...
        pl.savefig('stdpfiringrate3.eps', format='eps')
